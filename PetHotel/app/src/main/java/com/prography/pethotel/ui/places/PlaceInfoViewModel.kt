...
class PlaceInfoViewModel : ViewModel() {
    ...
    private val _hotelList = MutableLiveData<ArrayList<Hotel>>()
    val hotelList : LiveData<ArrayList<Hotel>>
        get() = _hotelList
    ...
    private fun getHotelLists(){
        try {
            uiScope.launch {
                val response = MainApi.petHotelRetrofitService.getHotelList()
                _hotelList.value = response as ArrayList<Hotel>
            }
        }catch (e : Exception){
            e.printStackTrace()
        }
